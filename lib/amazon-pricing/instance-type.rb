...
#   # type_of_instance = :ondemand, :light, :medium, :heavy
    ...
    def price_per_hour(category_type, type_of_instance, term = nil, deploy_type = nil)
      cat = get_category_type(category_type)
      cat.price_per_hour(type_of_instance, term, deploy_type) unless cat.nil?
    end

    # type_of_instance = :ondemand, :light, :medium, :heavy
    # term = :year_1, :year_3, nil
    def prepay(category_type, type_of_instance, term = nil, deploy_type = nil)
      cat = get_category_type(category_type)
      cat.prepay(type_of_instance, term, deploy_type) unless cat.nil?
    end

    # category_type = :mysql, :oracle, :sqlserver
    # type_of_instance = :ondemand, :light, :medium, :heavy
    def update_pricing(category_type, type_of_instance, json, deploy_type)
      cat = get_category_type(category_type)
      if cat.nil?
        cat = DatabaseType.new(self, category_type) if deploy_type
        cat = OperatingSystem.new(self, category_type) unless deploy_type
      ...
      if type_of_instance == :ondemand
        ...
        cat.set_price_per_hour(type_of_instance, nil, price, deploy_type)
      else
        json['valueColumns'].each do |val|
          price = coerce_price(val['prices']['USD'])
          case val["name"]
          when "yrTerm1"
            cat.set_prepay(type_of_instance, :year1, price, deploy_type)
          when "yrTerm3"
            cat.set_prepay(type_of_instance, :year3, price, deploy_type)
          when "yrTerm1Hourly"
            cat.set_price_per_hour(type_of_instance, :year1, price, deploy_type)
          when "yrTerm3Hourly"
            cat.set_price_per_hour(type_of_instance, :year3, price, deploy_type)
    ...
    @@Disk_Lookup = {
      ...
      'db.m2.xlarge' => 420, 'db.m2.2xlarge' => 850, 'db.m2.4xlarge' => 1690, 'db.m2.8xlarge' => 000
    }
    @@Platform_Lookup = {
