...
module AwsPricing
  ...
  class RdsPriceList < PriceBase
    ...
    def fetch_on_demand_rds_instance_pricing(url, type_of_rds_instance, db_type, on_demand_list)
      res = fetch_url(url)
      res['config']['regions'].each do |reg|
        region_name = reg['region']
        region = find_or_create_region(region_name)
        reg['types'].each do |type|
          type['tiers'].each do |tier|
            begin
              if on_demand_list == "multiAZ" and db_type == :mysql and type["name"] == "clusterHiMemDB"
                api_name, name = InstanceType.get_name("multiclusterHiMemDB", tier["name"], type_of_rds_instance != :ondemand)
              elsif db_type == :oracle or db_type == :sqlserver
                api_name, name = InstanceType.get_name("#{on_demand_list}-#{type['name']}", tier["name"], type_of_rds_instance != :ondemand)  
              else
                api_name, name = InstanceType.get_name(type["name"], tier["name"], type_of_rds_instance != :ondemand)
              end  
              region.add_or_update_instance_type(api_name, name, db_type, type_of_rds_instance, tier)              
            rescue UnknownTypeError
              $stderr.puts "WARNING: encountered #{$!.message}"
            end
          end
    ...
    def fetch_reserved_rds_instance_pricing(url, type_of_rds_instance, db_type, reserved_list)
      res = fetch_url(url)
      res['config']['regions'].each do |reg|
        region_name = reg['region']
        region = find_or_create_region(region_name)
        reg['instanceTypes'].each do |type|
          type['tiers'].each do |tier|
            begin
              if db_type == :oracle or db_type == :sqlserver
                if db_type == :sqlserver and reserved_list == 'byol'
                  api_name, name = InstanceType.get_name("sql-#{reserved_list}-#{type["type"]}", tier["size"], true)                
                else
                  api_name, name = InstanceType.get_name("#{reserved_list}-#{type["type"]}", tier["size"], true)                
                end                
              else
